...
public class CensusChgPlugin extends AjaxModelFactory{
	...
	private static String CHANGE_SQL = "SELECT * FROM census_change(?,?);";

	public CensusChgPlugin() {
	}
	...
	public String getHTMLResponse () {
		...
		try {
			htmlFile = (String)this.jq.getContent().getQueriesMap("tabCensusChg").get("Resource");
	...
	public String getJSONResponse () {		
		...
		jsonResultBuilder.append("{\"id\":\"estdata\",\"label\":\"Estimate Column\",\"type\":\"string\"},");
	...
	public String getSVGResponse () {
		...
		try {
			svgFile = (String)this.jq.getContent().getQueriesMap("chgUSMap_div").get("Resource");
	    } catch (NullPointerException npe) {
	...
	protected void buildCHANGE_SQL () {

  		String fromDate="";
  		String toDate="";
		
  		this.PSel.setPgSQLStatement(CHANGE_SQL);
		
		try {
			fromDate = (String)this.jq.getContent().getQueriesMap("tabCensusChg").get("FromDate");
			toDate = (String)this.jq.getContent().getQueriesMap("tabCensusChg").get("ToDate");
	    } catch (NullPointerException npe) {
